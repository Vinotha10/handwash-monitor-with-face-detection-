...
from face_recognition.models import SCRFD, ArcFace
from face_recognition.utils.custom_logging import setup_logging
from face_recognition.utils.helpers import compute_similarity, draw_bbox_info, draw_bbox
...
def frame_processor(frame: np.ndarray, detector: SCRFD, recognizer: ArcFace, face_db: FaceDatabase, colors: dict, params: argparse.Namespace):
    ...
    for bbox, (name, similarity) in zip(processed_bboxes, results):
        if name != "Unknown":
            ...
            draw_bbox(frame, bbox, (0, 255, 255))
...
if __name__ == "__main__":
    ...
    args.faces_dir = os.path.abspath(os.path.join(project_dir, "assets", "faces"))
    args.det_weight = os.path.abspath(os.path.join(project_dir, "weights", os.path.basename(args.det_weight)))
    args.rec_weight = os.path.abspath(os.path.join(project_dir, "weights", os.path.basename(args.rec_weight)))
    args.source = os.path.abspath(os.path.join(project_dir, "assets", os.path.basename(args.source)))
    try:
